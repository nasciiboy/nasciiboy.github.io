...
function loadJquery() {
    let body = document.querySelector('body'),
        ...
        hostedJquery = 'js-libs/jquery.min.js';
...
function createSquigSelect() {
    if ( squigsSites && document.querySelectorAll('header.header').length > 0 ) {
        // let squigsitesJson = mode === 'dev' ? 'squigsites.json' : 'https://squig.link/squigsites.json?' + cacheBuster,
        let squigsitesJson = 'squigsites.json',
        ...
        squigSelectGroupOfficial.setAttribute('label', 'Official');
        squigSelect.append(squigSelectGroupOfficial);
            ...
            squigSites.forEach(function(site) {
                ...
                dbs.forEach(function(db) {
                    let type = db.type,
                        ...
                        targetOptGroup = type === 'IEMs' ? squigSelectGroupIems : type === 'Headphones' ? squigSelectGroupHeadphones : type === 'Earbuds' ? squigSelectGroupEarbuds : type === 'Official' ? squigSelectGroupOfficial : NULL;
    ...
    $.getJSON(squigsitesJson, function(squigSites) {
        ...
        squigSites.forEach(function(site) {
            let name = site.name,
                ...
                url = 'https://' + username + '.squig.link',
                phoneBookJson = 'https://'+ username +'.squig.link/data/phone_book.json',
                dbs = site.dbs;
            ...
            dbs.forEach(function(db) {
                let siteContainer = document.createElement('section'),
                    ...
                    let folder = db.folder,
                        ...
                        phoneBookJson = username === 'superreview' && folder === '/' ? 'https://squig.link/data/phone_book.json' : username === 'superreview' && folder === '/headphones/' ? 'https://squig.link/headphones/data/phone_book.json' : 'https://'+ username +'.squig.link'+ folder +'data/phone_book.json';
...
function loadSquiglinkIntro() {
    let body = document.querySelector('body'),
        ...
        introScriptSrc = 'squiglink-intro.js';
